...
In particular, it is like a unary operation with a fixed parameter `a`, where only `b` participates in type propogation. -/
...
In particular, it is like a unary operation with a fixed parameter `b`, where only `a` participates in type propogation. -/
